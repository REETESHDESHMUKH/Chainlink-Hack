...
const Mines = () => {
  const { web3, account, contract, balance, updateBalance } =
  ...
  const startGame = async () => {
    ...
    try {
      ...
      const response = await contract.methods
        .rollDice(account, 6, 20)
        .send({ from: account });
      setTimeout(async () => {
        try {
          const random = await contract.methods
            .getRandom()
            .call({ from: account });
  ...
  const revealBlock = (id: number) => {
    ...
    if (newBlocks[id].hasMine) {
      ...
      setScore(
        Number(Number(score + 2 * (numMines / totalBlocks)).toPrecision(2))
      );
      ...
      <div className="flex flex-row">
            ...
            <div className="grid grid-cols-5 grid-rows-5 gap-1">
              {blocks.map((block) => (
                ...
                >
                  {block.revealed && (
                    <img
                      src={block.hasMine ? "/blast.png" : "/flag.png"}
                      alt="icon"
                      className="w-14 h-14"
                    />
                  )}
        ...
        <div className="flex-1 flex flex-col bg-gradient-to-r from-slate-100 to-slate-200 p-6">
          <div className="flex-1 flex flex-col pb-4 overflow-y-auto">
            <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
              ...
              <div className="mt-1 sm:mt-0 sm:col-span-2">
                <input
                  ...
                  onChange={(e) =>
                    setNumMines(
                      Math.min(24, Math.max(1, parseInt(e.target.value)))
                    )
                  }
                  min={1}
                  max={24}
            ...
            <div className="pt-2 text-xs text-gray-500 mt-1">
              1 ticket = 0.005 ETH
            </div>
            <div className="text-xs text-gray-500 mt-1">
              Max tickets: 200 (1 ETH)
            </div>
            ...
            {tickets >= 1 && tickets <= 200 && (
              <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                ...
                <div className="pt-2 sm:mt-0 sm:col-span-2 text-gray-500">
                  {calculateEthAmount(tickets)} ETH
                </div>
              </div>
            )}
